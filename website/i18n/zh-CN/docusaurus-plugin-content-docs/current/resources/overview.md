...
- **[Account](account)**（帐户）：查询帐户余额、交易计数和合约程式码
- **[Block](block)**（区块）：检索区块资讯和目前区块链高度
- **[Transaction](transaction)**（交易）：传送交易、检查状态和估算 Gas 成本
- **[Gas](gas)**：取得 Gas 价格和费用历史以最佳化交易定价

### 智慧合约资源

- **[Contract](contract)**（合约）：读取和写入智慧合约、部署新合约和查询事件日志
- **[ERC20](erc20)**：与 ERC20 代币合约互动（转帐、授权、余额）
- **[ERC721](erc721)**：管理 ERC721 NFT 操作（转帐、所有权、中继资料）
- **[ERC1155](erc1155)**：使用 ERC1155 多代币标准（批次操作、余额）

### 实用工具资源

- **[ENS](ens)**：解析以太坊名称服务网域名称和反向查询
- **[Signature](signature)**（签名）：签名和验证讯息和类型化资料（EIP-712）
- **[Utils](utils)**（工具）：格式化、编码和验证的实用函式
- **[Custom RPC](custom-rpc)**（自订 RPC）：使用任何方法和参数传送原始 RPC 请求
...
- 了解 [Transactions](transaction) 以传送 ETH
- 深入了解 [Contract](contract) 进行智慧合约互动
- 探索代币标准：[ERC20](erc20)、[ERC721](erc721)、[ERC1155](erc1155)
