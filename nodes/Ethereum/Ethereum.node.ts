...
function createWalletClient(publicClient: any, credentials: any) {
	const hasPrivateKey = credentials.privateKey && (credentials.privateKey as string).trim() !== '';
	const hasMnemonic = credentials.mnemonic && (credentials.mnemonic as string).trim() !== '';
	...
	if (hasPrivateKey) {
		...
		throw new Error('Either privateKey or mnemonic must be provided in wallet credentials');
	}
...
function parseBlockIdentifier(blockStr: string): any {
	if (blockStr === 'latest' || blockStr === 'earliest' || blockStr === 'pending') {
		return blockStr;
	}
	...
	description: INodeTypeDescription = {
		...
		properties: [
			...
			},
			{
				...
				default: false,
				description: 'Whether to use raw calldata instead of function name and parameters',
			},
			{
			...
			// ERC721: Token ID operations
			{
				...
				displayOptions: {
					show: {
						resource: ['erc721'],
						operation: ['ownerOf', 'transferFrom', 'safeTransferFrom', 'approve', 'getApproved', 'tokenURI'],
					},
				},
				default: '',
			},
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if (resource === 'account') {
					...
					if (blockTag === 'custom') {
						blockNumber = BigInt(this.getNodeParameter('blockNumber', i) as number);
					}
				...
				// ===========================================
					...
					if (operation === 'getBlock') {
						const identifierType = this.getNodeParameter('blockIdentifierType', i) as string;
						const includeTransactions = this.getNodeParameter('includeTransactions', i) as boolean;

						let block;
						if (identifierType === 'number') {
							const blockNumber = BigInt(this.getNodeParameter('blockNumber', i) as number);
				...
				// ===========================================
					...
					if (operation === 'sendTransaction') {
						...
						const walletClient = createWalletClient(publicClient, walletCredentials);
				...
				// ===========================================
					...
					if (operation === 'balanceOf') {
						...
						const hash = await walletClient.writeContract({
							...
							abi: ERC1155_ABI,
							functionName: 'safeTransferFrom',
							args: [from as `0x${string}`, to as `0x${string}`, BigInt(id), BigInt(amount), data as `0x${string}`],
							account: walletClient.account!,
							chain: undefined,
						});
						...
						const ids = JSON.parse(idsStr).map((id: string) => BigInt(id));
						const amounts = JSON.parse(amountsStr).map((amount: string) => BigInt(amount));

						const hash = await walletClient.writeContract({
							address: contractAddress as `0x${string}`,
							abi: ERC1155_ABI,
							functionName: 'safeBatchTransferFrom',
							args: [from as `0x${string}`, to as `0x${string}`, ids, amounts, data as `0x${string}`],
							account: walletClient.account!,
							chain: undefined,
						});
				...
				// ===========================================
					...
					if (operation === 'getGasPrice') {
						...
						responseData = {
							...
							reward: feeHistory.reward?.map((r: any) => r.map((v: any) => v.toString())),
						};
