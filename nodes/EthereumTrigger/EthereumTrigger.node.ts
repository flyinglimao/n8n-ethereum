...
export class EthereumTrigger implements INodeType {
	description: INodeTypeDescription = {
		...
		properties: [
			...
			// ===========================================
			{
				...
				default: '',
				placeholder: '0x..., 0x...',
				description: 'Contract addresses to monitor (comma-separated). Leave empty for all addresses.',
			},
			{
			...
			},
			{
				...
				default: 12000,
				description: 'How often to check for new transactions (in milliseconds)',
			},
		],
	};
	...
	async trigger(this: ITriggerFunctions): Promise<ITriggerResponse> {
			...
			try {
				if (event === 'newBlock') {
					...
					if (addressesStr.trim()) {
						addresses = addressesStr.split(',').map((addr) => addr.trim() as `0x${string}`).filter((addr) => addr);
					}
					...
					if (abiInput === 'abiEvent') {
						...
						logs = await publicClient.getLogs({
							address: addresses,
							event: abi.find((item: any) => item.type === 'event' && item.name === eventName),
					...
					const addresses = addressesStr.split(',').map((addr) => addr.trim().toLowerCase()).filter((addr) => addr);
					if (addresses.length === 0) {
						throw new NodeOperationError(this.getNode(), 'At least one address is required');
					}
					...
					for (let blockNum = startBlock; blockNum <= currentBlock; blockNum++) {
						...
						for (const tx of block.transactions) {
							...
							else if (direction === 'value') shouldEmit = (fromMatch || !!toMatch) && tx.value > 0n;
				...
				throw new NodeOperationError(this.getNode(), `Polling error: ${errorMessage}`);
			}
		};
		...
		const pollingInterval = this.getNodeParameter('pollingInterval', 12000) as number;
